...
socket.on("get-board", () => {
  console.log("board: ", MOVIMIENTOS);
  socket.emit("board", { board: MOVIMIENTOS });
});
...
const pintarCasillas = (casilla) => {
  ...
  casillaJugada.style.background =
    MOVIMIENTOS.length % 2 == 1 ? "#ff0000" : "#0000ff";
  casillaJugada.setAttribute(
    "jugador",
    `${MOVIMIENTOS.length % 2 == 1 ? "1" : "2"}`
  );
};
...
const ponerPieza = (columna) => {
  ...
  let columna_ = Array.from(
    document.querySelectorAll(`[columna='${columna}']`)
  );
  for (let casilla = 0; casilla < FILAS; casilla++) {
    ...
    if (
      columna_[casilla].getAttribute("ocupada") !== "1" &&
      casilla != FILAS - 1
    )
      continue;
    ...
    if (
      ...
      return [
        columna_[casilla].getAttribute("fila"),
        columna_[casilla].getAttribute("columna"),
      ];
    }
    ...
    return [
      columna_[casilla - 1].getAttribute("fila"),
      columna_[casilla - 1].getAttribute("columna"),
    ];
  }
};
  ...
  for (let fila in tablero) {
    for (let columna in tablero[fila]) {
      if (columna < COLUMNAS - 3) {
        ...
        let casilla2 = parseInt(
          tablero[fila][parseInt(columna) + 1].getAttribute("jugador")
        );
        let casilla3 = parseInt(
          tablero[fila][parseInt(columna) + 2].getAttribute("jugador")
        );
        let casilla4 = parseInt(
          tablero[fila][parseInt(columna) + 3].getAttribute("jugador")
        );
        if (
          casilla1 &&
          casilla1 === casilla2 &&
  ...
  for (let fila in tablero) {
    for (let columna in tablero[fila]) {
      if (fila < FILAS - 3) {
        let casilla1 = parseInt(tablero[fila][columna].getAttribute("jugador"));
        let casilla2 = parseInt(
          tablero[parseInt(fila) + 1][columna].getAttribute("jugador")
        );
        let casilla3 = parseInt(
          tablero[parseInt(fila) + 2][columna].getAttribute("jugador")
        );
        let casilla4 = parseInt(
          tablero[parseInt(fila) + 3][columna].getAttribute("jugador")
        );
        if (
          casilla1 &&
          casilla1 === casilla2 &&
  ...
  for (let fila in tablero) {
    for (let columna in tablero[fila]) {
      if (fila < FILAS - 3 && columna < COLUMNAS - 3) {
        let casilla1 = parseInt(tablero[fila][columna].getAttribute("jugador"));
        let casilla2 = parseInt(
          tablero[parseInt(fila) + 1][parseInt(columna) + 1].getAttribute(
            "jugador"
          )
        );
        let casilla3 = parseInt(
          tablero[parseInt(fila) + 2][parseInt(columna) + 2].getAttribute(
            "jugador"
          )
        );
        let casilla4 = parseInt(
          tablero[parseInt(fila) + 3][parseInt(columna) + 3].getAttribute(
            "jugador"
          )
        );
        if (
...
const crearCartelGanador = (ganador) => {
  ...
  cartelGanador.style.color =
    parseInt(ganador) % 2 == 1 ? "#ff0000" : "#0000ff";
...
const juego = (e) => {
  ...
  let turno = (MOVIMIENTOS.length + 1) % 2 == 1 ? 0 : 1;
  console.log(document.getElementById("id").innerHTML);
  const id = document.getElementById("id").innerText;
  socket.emit("move", { fila, columna, id, turno });
};

socket.on("valid-movement", ({ fila, columna }) => {
  let casilla = tablero[fila][columna];
...
socket.on("new-board", ({ board }) => {
  //console.log(board, MOVIMIENTOS, board === MOVIMIENTOS);
  if (MOVIMIENTOS.length) {
    return;
  }
  console.log("new-board: ", board);
  for (let mov in board) {
    let casillaJugada = board[mov];
    ponerPieza(casillaJugada[1]);
    agregarMovimiento(casillaJugada);
    pintarCasillas(casillaJugada);
  }
  console.log(board);
